...
class Options:
    ...
    def _set_Navigate(self, Action):
        ...
        if int_CurrentLine < 0:
           lst_HostsCheck = self._get_CheckList_Slice(-1, call_str_Title, call_InitialLine, call_InitialColumn, lst_Options, ClearLine=True, Options_sets=lst_Options_sets)
           int_CurrentLine = 0
    ...
    def _get_CheckList_Slice(self, int_Acao, str_Title, InitialLine, InitialColumn, Options, ClearLine=True, Options_sets=[], pos_Lista=0): #int_Acao, par_lst_Print, pos_Lista=0):
        ...
        sys.stdout.write("\x1b7\x1b[%d;%df%s\x1b8" % (int_InitialLine + 1, 2, ' '))
        ...
        for idx,i in enumerate(range(int_posTOP, int_posDOWN)):
            ...
            if ind_Origem == 1:
               sys.stdout.write("\x1b7\x1b[%d;%df%s\x1b8" % (int_InitialLine + idx, int_InitialColumn + 2, '   [ ] - ' + Options[i]))
            else:
               sys.stdout.write("\x1b7\x1b[%d;%df%s\x1b8" % (int_InitialLine + idx, int_InitialColumn + 2, '   ( ) - ' + Options[i]))
            sys.stdout.flush()
